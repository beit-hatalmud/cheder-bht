...
async function renderLessonsYodlov() {
  ...
  _lvents = (evRes.data || []).filter(e => e['קטגוריה'] === LESSONSYODLOV_CAT);
  _lvents.sort((a,b) => new Date(b['תאריך']) - new Date(a['תאריך']));
  fillLessonsYodlovFilters();
  drawLessonsYodlovEvents(_lvents);
  document.getElementById('r-fstudent').onchange = applyLessonsYodlovFilters;
}
